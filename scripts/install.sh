...
#
# Everything executable lives in main(), invoked on the LAST line — so a
# truncated `curl | bash` stream defines functions but executes nothing.
#
...
main() {
  ...
  # --- platform detection ------------------------------------------------------
  # Yields a Rust target triple for the 4 published assets, or empty for
  # "unsupported here" (then we try the cargo fallback). Windows is refused.
  ...
  if [ -n "$TARGET" ]; then
    ...
    # Atomic install: stage inside the destination dir, then rename over the
    # final name — a running old binary keeps its inode (no ETXTBSY) and no
    # reader ever sees a partially-written file.
    mkdir -p "$INSTALL_DIR"
    staged="$INSTALL_DIR/.jerrycan.new.$$"
    cp "$tmp/jerrycan" "$staged"
    chmod 755 "$staged"
    mv -f "$staged" "$INSTALL_DIR/jerrycan"
  ...
  if [ "$TARGET" != "cargo-install" ] && [ "${JERRYCAN_NO_MODIFY_PATH:-0}" != "1" ]; then
    ...
    if [ -f "$rc" ] && grep -qF "$marker" "$rc" 2>/dev/null; then
      if grep -qF "$INSTALL_DIR" "$rc" 2>/dev/null; then
        ...
        log "note: $rc has the jerrycan PATH marker but references a different dir than $INSTALL_DIR — your PATH may still point at the old install location (rc left untouched)"
      fi
    else
      {
        printf '\n%s\n' "$marker"
