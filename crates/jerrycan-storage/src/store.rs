...
mod tests {
    ...
    fn validate_key_rejects_traversal_and_junk() {
        ...
        for bad in [
            "", "/abs", "a//b", "../up", "a/../b", "a/./b", "trail/", "spa ce", "quo\"te",
        ] {
    ...
    async fn memory_store_round_trips_and_deletes() {
        ...
        s.put("b", "k.txt", Bytes::from_static(b"hello"), "text/plain")
            .await
            .unwrap();
        assert_eq!(
            s.get("b", "k.txt").await.unwrap(),
        ...
        assert!(
            s.presign_get("b", "k.txt", std::time::Duration::from_secs(60))
                .await
                .unwrap()
                .is_none()
        );
    ...
    async fn local_store_round_trips_under_its_root() {
        ...
        .unwrap();
        assert!(
            dir.path().join("avatars/u/1.png").is_file(),
            "bytes land under root/bucket/key"
        );
        assert_eq!(
            s.get("avatars", "u/1.png").await.unwrap(),
            Bytes::from_static(b"png")
        );
        s.delete("avatars", "u/1.png").await.unwrap();
        assert_eq!(
            s.get("avatars", "u/1.png").await.unwrap_err().code(),
            "JC0404"
        );
