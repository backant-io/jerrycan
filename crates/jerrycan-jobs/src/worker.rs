...
pub struct JobConfig {
    ...
    /// How long a lease is held before it is reclaimable (e.g. 30s).
...
impl Worker {
    ...
    pub async fn tick(&self, queue: &str, now: SystemTime, base_ctx: TaskContext) -> Result<usize> {
        let jobs = self
            .store
            .lease(queue, now, self.config.lease_duration, self.config.batch)
            .await?;
        let n = jobs.len();
        for job in jobs {
            let Some(task) = self.dispatch.get(&job.name).cloned() else {
                eprintln!(
                    "jerrycan-jobs: no handler for job '{}' (id {}) — dead-lettering",
                    job.name, job.id
                );
    ...
    async fn fail(&self, job: &Job, now: SystemTime) -> Result<()> {
        if job.attempts >= job.max_attempts {
            eprintln!(
                "jerrycan-jobs: job '{}' (id {}) dead-lettered after {} attempts",
