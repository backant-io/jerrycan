...
fn scaffold_and_apply_fixtures(app: &Path) {
    ...
    // Cargo feature patch (REQUIRED): the design's dependencies array can't
    // express oauth/mock-idp, so the integrations module won't compile without
    // them. Append the two features to the generated workspace jerrycan dep.
    let cargo = app.join("Cargo.toml");
    let before = r#"features = ["db", "validate", "auth", "observe", "jobs"]"#;
    ...
    assert!(
        txt.contains(before),
        "the scaffolded Cargo.toml must carry the documented feature set to patch"
    );
