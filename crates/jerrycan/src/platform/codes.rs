...
pub const REGISTRY: &[CodeInfo] = &[
    ...
    CodeInfo {
        ...
        cause: "a `belongs_to` fk column (issue #119) is unbuildable: two `belongs_to` on the same entity derive the SAME fk column — two un-aliased refs to one target (both `snake(entity)_id`), or an `as` alias whose `{as}_id` equals another belongs_to's fk column — so the generated Model would carry a DUPLICATE fk field and the migration a duplicate column; OR an `{as}_id` (or the default `snake(entity)_id`) COLLIDES with a declared field name or the pk `id`; OR the `as` alias is MALFORMED (not snake_case `^[a-z][a-z0-9_]*$`, so the derived column and Rust field would be invalid); OR the `as` lands on a RESERVED fk the target doesn't own — the identity fk `user_id` (in an auth design) or the tenancy fk — which would hijack per-user/tenant scoping and fail as opaque generated Rust; OR (issue #257) the belongs_to that ANCHORS an entity to the tenancy entity is itself ALIASED AWAY from the canonical tenancy fk — `tenant_path` hardcodes the anchor's fk as the canonical `fk_column(tenancy.entity)` and every generated tenant query (the membership JOIN, `require_membership_role`, the scoped accessors) references it, so an aliased anchor (`{Tenant} as \"x\"` → `x_id`) breaks all tenant scoping (a runtime `no such column` on a grandchild, an E0599 on a direct child). The alias exists precisely so two references to one entity (a ledger Transfer's from_account/to_account, a self-referential Comment's parent) get DISTINCT fk columns and distinct DDL constraint names",
        fix: "give each `belongs_to` a distinct fk column: add an `as` alias to at least one of two refs to the same entity (`{ \"entity\": \"Account\", \"as\": \"from_account\" }` → `from_account_id`), so no two fk columns and no fk-vs-field/pk names collide; make every `as` snake_case (`^[a-z][a-z0-9_]*$`); never alias onto the reserved `user_id` or tenancy fk (only the identity/tenancy entity owns those); and never alias the belongs_to that anchors an entity to the tenancy entity — keep its fk the canonical `snake(tenancy.entity)_id` (rename or drop the `as`). A single un-aliased `belongs_to` per target needs no `as` (byte-identical)",
        doc: "jerrycan docs designing",
    },
    CodeInfo {
        code: "JC0561",
