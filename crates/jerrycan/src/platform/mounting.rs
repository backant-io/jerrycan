...
pub fn expected_main(design: &Design) -> String {
    ...
    // db mode declares the tool-owned migrations module and the AGENT-owned boot
    // module (issue #12): `boot::on_boot(&db)` runs after migrations, before the
    // app serves — the hook for idempotent startup work (a dev seed, etc.).
...
mod tests {
    ...
    fn expected_main_wires_the_boot_hook_after_migrations_in_db_mode() {
        let main = expected_main(&jobs_design());
        assert!(main.contains("mod boot;"), "declares the boot module: {main}");
        let boot = main
