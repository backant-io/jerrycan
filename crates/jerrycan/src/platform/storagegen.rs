...
pub fn acceptance_rs(design: &Design) -> String {
    ...
    // NAME is kept for both models; the credential HEADER threaded into requests is
    // renamed cookie→authorization below. Both are no-ops under `session`, so
    // session/none storage output stays byte-identical.
    ...
    let tests = tests.replace(
        "(\"cookie\", &test_cookie_for",
        &format!("(\"{hk}\", &test_cookie_for"),
    );
...
mod tests {
    ...
    fn jwt_storage_threads_bearer_not_cookie() {
        ...
        assert!(
            !a.contains("jerrycan_session=") && !a.contains("(\"cookie\", &test_cookie_for"),
