...
/// 404s a non-member (an INVISIBLE key) while 403ing a MEMBER with the wrong role
...
/// the key's existence. The membership-scoped `remove_for_memberships` then deletes
/// only a key in the caller's set; an unknown id in the caller's own tenant → 404.
