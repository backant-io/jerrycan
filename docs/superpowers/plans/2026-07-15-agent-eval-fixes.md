...
**Branch:** `feat/jwt-bearer-guards` · **Plan appended after the auth-architecture scout returns** (currently mapping: session guard emission sites, existing JWT infra in jerrycan-auth/realtime, testgen auth, SessionUser extraction, secret config). Scope decided: `auth.model: "jwt"` generates Bearer-token verification guards for REST routes; login issues the token; generated tests authenticate via `Authorization: Bearer`. Must respect the known `SessionUser.id`-is-String contract. If the scout reveals this is larger than a single package (e.g. session-store coupling), the plan gets split and re-confirmed with the maintainer before dispatch.
