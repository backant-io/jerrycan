...
**Tech stack:** hyper HTTP/1 upgrade (`.with_upgrades()`, new in core) + `tokio-tungstenite 0.30` (no TLS features — it rides hyper's already-established socket); `postgres-protocol 0.6` + a small hand-rolled replication socket over tokio TCP + rustls (ring, webpki-roots — mirrors the oauth client stance); sqlx `PgListener` (already in tree) for the trigger fallback; the existing workspace `redis` for the multi-node bus. sqlx/sea-orm stays the sole data-layer client.
